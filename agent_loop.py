...
class Agent:
    ...
    def __init__(self, client: OpenAI, model: str, system_prompt: str,
        ...
        self._turns_since_memory = 0##5
        self._iters_since_skill = 0##8
        self._enable_prompt_caching = False  # set via configure_caching()
        self._caching_provider = "auto"
        self._log_cache_usage = True
        self._cache_ttl = "1h"
        self._caching_base_url = ""
        ...
        self._memory_nudge_interval = 5##5
        self._skill_nudge_interval = 8##8
    ...
    def configure_caching(self, enable: bool = False, cache_ttl: str = "5m",
                          provider: str = "auto", base_url: str = "",
                          log_usage: bool = True):
        """Configure prompt caching (Chapter 6). Injection only on Anthropic APIs."""
        self._enable_prompt_caching = enable
        self._cache_ttl = cache_ttl
        self._caching_provider = provider
        self._caching_base_url = base_url or ""
        self._log_cache_usage = log_usage
    ...
    def _call_llm(self):
        ...
        try:
            ...
            if self._compressor:
                ...
                if est >= threshold:
                    # Flush memories before compression (user-role sentinel)
                    from contextengineering.flush_memories_beforecompression import flush_memories
                    flush_memories(self, self.messages)##先刷盘-后压缩##===================================##===================================##===================================
                    self.messages = self._compressor.maybe_compress(self.messages)##先刷盘-后压缩##===================================
            ...
            if self._enable_prompt_caching:
                api_messages = apply_prompt_caching(
            ...
            # Strategy decides whether to pass tools as API param or inject them into the system prompt
    ...
    def run(self, user_input: str) -> str:
        ...
        self._turns_since_memory += 1##====memory
        ...
        for i in range(self.max_iterations):#15
            ...
            self._iters_since_skill += 1##====skill
            for tc in tool_calls:
                # Reset nudge counter if agent uses the tool voluntarily
                if tc.name == "memory":
                    self._turns_since_memory = 0##====memory
                elif tc.name == "skill_manage":
                    self._iters_since_skill = 0##====skill

                result = self._execute_tool(tc.name, tc.arguments)##=======================
                result_msg = self._strategy.build_tool_result_msg(tc, result)##===================================
                self.messages.append(result_msg)##========
                self._persist_message("tool", result, tool_name=tc.name,tool_call_id=tc.id)##========
        else:
        ...
        should_review_memory = (
            ...
            and self._turns_since_memory >= self._memory_nudge_interval##5 ##====memory
        )
        ...
        should_review_skills = (
            ...
            and self._iters_since_skill >= self._skill_nudge_interval##8 ##====skill
        )
