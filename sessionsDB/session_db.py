...
class SessionDB:
    ...
    def _create_tables(self):
        self.conn.executescript("""
            CREATE TABLE IF NOT EXISTS sessionsDB (
                id TEXT PRIMARY KEY,
    ...
    def create_session(self, source: str = "cli",
        ...
        self.conn.execute(
            "INSERT INTO sessionsDB (id, source, started_at, system_prompt) "
            "VALUES (?, ?, ?, ?)",
    ...
    def get_session(self, session_id: str) -> Optional[dict]:
        row = self.conn.execute(
            "SELECT id, source, started_at, system_prompt FROM sessionsDB WHERE id=?",
            (session_id,)
        ).fetchone()
        if not row:
            return None
    ...
    def end_session(self, session_id: str):
        self.conn.execute(
            "UPDATE sessionsDB SET ended_at=? WHERE id=?",
    ...
    def append_message(self, session_id: str, role: str, content: str,
        ...
        self.conn.execute(
            "UPDATE sessionsDB SET message_count = message_count + 1 WHERE id = ?",
            (session_id,)
        )
        self.conn.commit()
    ...
    def ____跨会话sessions(self):
        pass
    def search(self, query: str, limit: int = 20) -> list[dict]:
        """Full-text search across all sessionsDB."""
        ...
        try:
            rows = self.conn.execute("""
                ...
                JOIN sessionsDB s ON s.id = m.session_id
